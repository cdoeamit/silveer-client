...
const [silverPaymentData, setSilverPaymentData] = useState({
  ...
  const handleAddPayment = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.post(`/billing/sales/${sale.id}/payment`, paymentData);
...
const handleAddSilverPayment = async (e) => {
  ...
  try {
    const response = await api.post(`/billing/sales/${sale.id}/silver-payment`, silverPaymentData);
            ...
            <div className="border-l-4 border-purple-500 bg-purple-50 p-4 rounded">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                  <span className="font-medium">₹{parseFloat(sale.subtotal).toFixed(2)}</span>
                </div>
                {sale.gstApplicable && (
                  <>
                    <div className="flex justify-between">
                      <span>CGST (1.5%):</span>
                      <span className="font-medium">₹{parseFloat(sale.cgst).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>SGST (1.5%):</span>
                      <span className="font-medium">₹{parseFloat(sale.sgst).toFixed(2)}</span>
                    </div>
                  </>
                )}
...
{showSilverPaymentModal && (
  ...
  >
    <div 
      ...
      onClick={(e) => e.stopPropagation()}  // ✅ This stops the click from closing the modal
    >
