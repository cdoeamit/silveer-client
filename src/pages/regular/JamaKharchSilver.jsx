...
const JamaKharchSilver = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const response = await api.get(`/regular-jama-kharch/silver?type=${activeTab}`);
  ...
  const handleFineSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/regular-jama-kharch/silver', {
        ...fineFormData,
        type: activeTab,
        silverType: 'fine'
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/regular-jama-kharch/silver/${id}`);
                      ...
                      <tr key={txn.id} className="hover:bg-gray-50">
                        ...
                        <td className="border p-2">
                          <span className={`px-2 py-1 rounded text-sm ${
                            txn.silverType === 'raw' ? 'bg-yellow-100 text-yellow-800' : 'bg-purple-100 text-purple-800'
                          }`}>
